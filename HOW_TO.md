...
- [ ] **Verify CLI Support**: Only tools with a programmatic CLI/headless mode can be used as **subagents** (e.g., Antigravity, Gemini, Codex, Kiro).
- [ ] **IDE-only or Orchestrator Tools**: Orchestrators (e.g., Claude Code, Antigravity 2.0) can **learn and use** these skills to delegate *to* other subagents, while programmatic CLIs (like Antigravity CLI `agy` using `.agents/skills/`) can act as subagents themselves.

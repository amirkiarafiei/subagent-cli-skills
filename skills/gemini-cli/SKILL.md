...
- **Heavy Tasks**: `pro` (Alias for latest `gemini-3.1-pro-preview` or `gemini-3.1-pro`). Use for deep reasoning and large-scale refactors.
- **Strategy**: Always default to `gemini-3.6-flash` to minimize costs. Only escalate to `pro` for critical architecture tasks after verifying latest version and cost via search.
...
```bash
gemini "[prompt with @paths as needed]" --yolo -o text -m gemini-3.6-flash 2>&1
```
...
- **Never carry a flag habit across CLIs.** The same short flag means different things in different
  ...
  hangs the run forever. Confirm every flag against `gemini --help`.
- **An unrecognized-flag error means this skill is stale, not that the task is impossible.** Run
  `gemini --help`, proceed with the flags that exist, and tell the user which line here needs updating.

## Quick prompts

- **Delegate implementation**: `gemini "GOAL: [goal] | DECISIONS: [decisions] | SCOPE: [paths] | CONSTRAINTS: [constraints] | VERIFICATION: [test_command] | OUTPUT: [format]" --yolo -o text -m gemini-3.6-flash`
